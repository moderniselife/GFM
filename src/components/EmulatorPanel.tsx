...
import { CheckboxGroup, Checkbox, Wrap, WrapItem, VStack, HStack } from '@chakra-ui/react';
  ...
  const handleEmulatorAction = async (action: 'start' | 'stop' | 'restart') => {
    ...
    try {
      const response = await fetch(
        `http://localhost:3001/api/firebase/emulators?dir=${encodeURIComponent(projectDir)}`,
        {
          method: 'POST',
          headers: {
