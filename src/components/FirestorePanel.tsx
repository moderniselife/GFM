  ...
  return (
    <>
      <Tr>
        ...
        <Td>
          <Text noOfLines={isExpanded ? undefined : 2}>{JSON.stringify(doc.data)}</Text>
        </Td>
        <Td>
          <HStack spacing={2}>
                  ...
                  <Box key={key}>
                    {!isCollapsed ? (
                      ...
                      <>
                        ...
                        <Box pl={4}>{renderValue(val, [...currentPath, key])}</Box>
                      </>
                    )}
                  </Box>
                ))}
              </VStack>
              <Text color="orange.500">{Array.isArray(value) ? ']' : '}'}</Text>
            </>
          )}
        </Box>
      );
...
export function FirestorePanel() {
  ...
  const handleDelete = async (docId: string) => {
    try {
      ...
      const response = await fetch(
        `http://localhost:3001/api/firebase/firestore/delete?projectId=${encodeURIComponent(projectId)}`,
        {
          method: 'POST',
  ...
  const handleDocumentUpdate = async (newData: any) => {
    try {
      ...
      const path = [...currentPath, collection].join('/');
      const response = await fetch(
        `http://localhost:3001/api/firebase/firestore/update?projectId=${encodeURIComponent(projectId)}`,
        {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
      ...
      <VStack spacing={4} align="stretch">
        <HStack>
          ...
          <Input
            ...
            size="sm"
          />
          <Button size="sm" colorScheme="blue" onClick={() => fetchData()} isLoading={loading}>
            Query
          </Button>
        </HStack>
        ...
        {isDocument ? (
          <Box borderWidth="1px" borderRadius="md" p={4}>
            ...
            <VStack align="stretch" spacing={4}>
              <Box borderRadius="md" overflowX="auto" backgroundColor="gray.50" p={4}>
                <EditableJSON
