...
export function AnalyticsPanel() {
  ...
  const fetchAnalytics = async () => {
    ...
    setError(null);
    try {
      const currentProjectId = await manager.getCurrentProjectId();
      if (!currentProjectId) {
        throw new Error('No active project found');
      }
      setProjectId(currentProjectId);
      const response = await fetch(
        `http://localhost:3001/api/ga4/data?projectId=${encodeURIComponent(currentProjectId)}&timeRange=${timeRange}&projectDir=${encodeURIComponent(projectDir)}`
      );
      ...
      <VStack spacing={4} align="stretch">
        ...
        {analyticsData && (
          <>
            ...
            <Box borderWidth="1px" borderRadius="lg" height="300px" p={4}>
              <Line
                data={chartData}
                options={{
                  responsive: true,
            ...
            <Box>
              <Text fontWeight="bold" mb={4}>Top Pages</Text>
              <VStack spacing={4} align="stretch">
