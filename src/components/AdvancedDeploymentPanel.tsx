...
export function AdvancedDeploymentPanel() {
  ...
  const loadFirebaseConfig = async () => {
    ...
    try {
      ...
      if (parsedConfig.hosting) {
        ...
        hostings.forEach((hosting: { site?: string; target?: string; public?: string }, index: number) => {
          options.push({
            type: 'hosting',
      ...
      if (parsedConfig.functions) {
        ...
        functions.forEach((func: { source?: string; codebase?: string }, index: number) => {
          options.push({
            type: 'functions',
      ...
      if (parsedConfig.storage) {
        ...
        storages.forEach((storage: { rules?: string }, index: number) => {
          if (storage.rules) {
            options.push({
              type: 'storage',
  ...
  const handleToggleAll = (type: string, checked: boolean) => {
    setDeploymentOptions(prev =>
      prev.map(option => (option.type === type ? { ...option, checked } : option))
    );
  };
  ...
  const groupedOptions = useMemo(() => {
    return deploymentOptions.reduce(
      (acc, option) => {
        if (!acc[option.type]) {
          acc[option.type] = [];
        }
    ...
    <Panel title="Advanced Deployment">
      <Box>
              ...
              <AccordionPanel>
                <VStack align="stretch" spacing={2}>
                  <Checkbox
                    ...
                    isIndeterminate={
                      options.some(opt => opt.checked) && !options.every(opt => opt.checked)
                    }
                  ...
                  {options.map((option, index) => (
                    <Checkbox
                      ...
                      onChange={() =>
                        handleToggleOption(
                          deploymentOptions.findIndex(
                            opt => opt.type === option.type && opt.name === option.name
                          )
                        )
                      }
                      ml={4}
        ...
        <HStack mt="auto" pt={4} spacing={2} width="100%">
          ...
          {loading && (
            <Button flex={1} colorScheme="red" onClick={handleCancel} variant="outline">
              Cancel
            </Button>
          )}
        </HStack>
